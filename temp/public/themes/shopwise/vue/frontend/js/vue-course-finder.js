...
var clientApp = new Vue({
    ...
    data: {
        ...
        types: window.types,
        filters: {
            keyword: ( window.filters.keyword)?window.filters.keyword:'',
            format: '',
            location_id: ( window.filters.location_id)?window.filters.location_id:'',
            duration: 0,
            min_price: '',
            max_price: '',
            from: ( window.filters.from)?window.filters.from:'',
            to: ( window.filters.to)?window.filters.to:'',
            categories:  ( window.filters.categories)?window.filters.categories:[],
        },// window.filters,
    ...
    methods: {
        ...
        onSelectCategory: function (category) {
            console.log("Added");
            let index = this.filters.categories.findIndex(item => item.id == category.id);
            //this.filters.categories[index].checked = true;
                ...
                for (i = 0; i < locations.length; i++) {
                    ...
                    google.maps.event.addListener(marker, 'click', (function (marker, i) {
                        ...
                        return function () {
                            ...
                            self.filters.location_id=marker.location_id;
                            self.search();
                            setTimeout(function(){
                                $('body,html').animate({ scrollTop: 700 }, 500);//$('body').height()
                            },300);
