...
class DoctorRepository extends RepositoriesAbstract implements DoctorInterface
{
    ...
    public function getAvailableApointments(int $id = 1)
    {
        $thisweek = [
            [
                'e_day' => $this->getEDay(0),
                'a_day' => $this->getADay(0),
                't_day' => $this->getADayTR(0),
                'date'  => $this->getDate(0)
            ],
            [
                'e_day' => $this->getEDay(1),
                'a_day' => $this->getADay(1),
                't_day' => $this->getADayTR(1),
                'date'  => $this->getDate(1)
            ],
            [
                'e_day' => $this->getEDay(2),
                'a_day' => $this->getADay(2),
                't_day' => $this->getADayTR(2),
                'date'  => $this->getDate(2)
            ],
            [
                'e_day' => $this->getEDay(3),
                'a_day' => $this->getADay(3),
                't_day' => $this->getADayTR(3),
                'date'  => $this->getDate(3)
            ],
            [
                'e_day' => $this->getEDay(4),
                'a_day' => $this->getADay(4),
                't_day' => $this->getADayTR(4),
                'date'  => $this->getDate(4)
            ],
            [
                'e_day' => $this->getEDay(5),
                'a_day' => $this->getADay(5),
                't_day' => $this->getADayTR(5),
                'date'  => $this->getDate(5)
            ],
            [
                'e_day' => $this->getEDay(6),
                'a_day' => $this->getADay(6),
                't_day' => $this->getADayTR(6),
                'date'  => $this->getDate(6)
            ],
        ];
        ...
        for ($i = 0; $i <= 6; $i++) {
            $thisweek[] = [
                'e_day' => $this->getEDay($i),
                'a_day' => $this->getADay($i),
                't_day' => $this->getADayTR($i),
                'date'  => $this->getDate($i)
            ];
        }
        ...
        foreach ($thisweek as $day) {
            if (count($this->getDayAvailableApointments($id, $day['e_day'], $day['date']))) {
                ...
                $day['t_name'] = $day['t_day'] . ' ' . $day['date'];
                $appointment[] = ['e_name' => $day['e_name'], 'a_name' => $day['a_name'],  't_name' => $day['t_name'] ];
            }
        }
    ...
    function getADayTR($days)
    {
        $date = Carbon::now()->addDays($days);
        $day = date('l', strtotime($date));

        $find = ["Saturday", "Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday"];
        $replace = ["Cumartesi", "Pazar", "Pazartesi", "Salı", "Çarşamba", "Perşembe", "Cuma"];
